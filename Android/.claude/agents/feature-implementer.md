...
목표는 1커밋 단위로 변경을 작게 유지하고, :core 불변식과 테스트를 함께 업데이트하는 것이다.

## 필수 준수 규칙

- rules 문서(architecture/data-model/widget/testing/performance) 우선
- Gradle 설정 파일은 수정하지 않는다(필요하면 별도 슬라이스로 분리 요구).
